...
export const chatApi = {
  ...
  getConversations: (params: ChatQueryParams) => {
    return http.get<{ list: Conversation[]; total: number }>('/admin/chat/conversations', params);
  },

  // 获取会话详情
  getConversation: (conversationId: number) => {
    return http.get<Conversation>(`/admin/chat/conversations/${conversationId}`);
  },

  // 获取消息列表
  getMessages: (params: ChatQueryParams) => {
    return http.get<{ list: ChatMessage[]; total: number }>('/admin/chat/messages', params);
  },

  // 获取用户聊天记录
  getUserMessages: (userId: number, params?: { page?: number; pageSize?: number }) => {
    return http.get<{ list: ChatMessage[]; total: number }>(`/admin/chat/user/${userId}/messages`, params);
  },

  // 获取聊天统计
  getStatistics: (params?: { startDate?: string; endDate?: string }) => {
    return http.get<ChatStatistics>('/admin/chat/statistics', params);
  },
